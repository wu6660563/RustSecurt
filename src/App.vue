...
import { cancelRecoveryScan, clearAccessPassword, hideFile, hideFolder, listItems, passwordConfigured, recoverMarkedItem, restoreItem, setAccessPassword, startRecoveryScan, verifyPassword, type HiddenItem, type RecoveryCandidate, type RecoveryScanUpdate } from './api/filehide'
...
const select = async (folder: boolean) => {
  try {
    const path = await open({ directory: folder, multiple: false, title: folder ? '选择要隐藏的文件夹' : '选择要隐藏的文件' })
    if (!path || Array.isArray(path)) return
    busy.value = true
    await (folder ? hideFolder(path) : hideFile(path))
    await refresh(); tab.value = 'active'; message.value = '已隐藏：属性已保存，可随时恢复。'
...
onMounted(async () => { try { passwordEnabled.value = await passwordConfigured(); appUnlocked.value = !passwordEnabled.value; if (appUnlocked.value) await refresh() } catch (error) { message.value = String(error) } })
onBeforeUnmount(() => { unlistenRecovery?.() })
</script>
